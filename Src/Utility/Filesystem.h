...
namespace QQ
{
	...
	*/
    ...
    // 通过文件名列表文件读取文件名列表,每一行格式：文件路径
    DLL_EXPORTS void GetFileNameListByTxt(const string &pathOfFileNameList, std::vector<string> &result);
	...
	/* 适合大数据量的读取(>500w),大数量使用WriteFileNameListToFile容易导致内存溢出程序崩溃
	不支持递归遍历且只支持单个扩展名(使用空字符串表示返回所有结果)
	addedPrefix:需要被添加的前缀

	测试：windows,i7-7700,32G,读取约1000w的文件名列表:
		WriteFileNameListToFile2:80s
		WriteFileNameListToFile:程序崩溃
	*/
	DLL_EXPORTS void WriteFileNameListToFile2(const string &directory, const string &extension, const string &pathOfFileNameList, const string &addedPrefix = "");
