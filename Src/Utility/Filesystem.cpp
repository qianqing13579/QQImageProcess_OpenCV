...
# include <sys/stat.h>
    ...
    void GetFileNameListByTxt(const string &pathOfFileNameList, std::vector<string> &result)
    {
        string temp=pathOfFileNameList;
        // read all lines
        std::ifstream fileNameList(temp,ios::in);
        std::vector<string> allLines;
        string line;
        while(getline(fileNameList,line))
        {
            if(!line.empty())
            {
	...
	void WriteFileNameListToFile2(const string &directory, const string &extension, const string &pathOfFileNameList, const string &addedPrefix)
	{
		std::ofstream fileNameList(pathOfFileNameList, ios::out);
		...
		if ((dir = opendir(directory.c_str())) != 0)
		{
			...
			try
			{
				...
				while ((ent = readdir(dir)) != 0)
				{
					...
					{
						// ���ļ���д���ļ�
						fileNameList << addedPrefix+name << endl;
					}
